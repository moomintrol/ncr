...
class Car
{
    ...
    // public static function carFilter($model, $status)
    // {
    //     if($model=="Все"){
    //         $model = '%%';
    //     }
    ...
    //     $query = Connection::make()->prepare("SELECT cars.*, bodies.name as body, models.name as model, statuses_cars.name as satus_car FROM cars INNER JOIN automobiles ON cars.automobiles_id = automobiles.id INNER JOIN bodies ON bodies.id = automobiles.body_id INNER JOIN models ON models.id = automobiles.model_id INNER JOIN statuses_cars ON statuses_cars.id = cars.status_car_id WHERE models.name LIKE :model AND statuses_cars.name LIKE :status");
    //     $query->execute([
    //         ':model' => $model,
    //         ':status' => $status
    //     ]);
    //     return $query->fetchAll();
    // }
    // UPDATE cars SET status_car_id = 1 WHERE (SELECT products_orders.car_id FROM products_orders WHERE products_orders.order_Id = :order_Id AND products_orders.car_id = :car_id) = :car_id AND (SELECT products_orders.order_id FROM products_orders WHERE products_orders.order_Id = :order_Id AND products_orders.car_id = :car_id)
}
